...
# Elasticsearch setup
es = Elasticsearch(['http://http://localhost:9200'])

def action(md5, md5main):

  # Create DXL configuration from file
  CONFIG_FILE = "/home/kai/Documents/Pen/OpenDXL-ATD-MAR-Elasticsearch/config/dxlclient1.config"
  ...
  with DxlClient(config) as client:
    ...
    if results_context.has_results:
         ...
         for item in results[ResultConstants.ITEMS]:
                ...
                item[u'Customer'] = {u'name': 'customer1'} 
                res = es.index(index='marc1', doc_type='response', body=item)
